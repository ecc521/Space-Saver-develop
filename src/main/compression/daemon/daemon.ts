...
try {
  const devPath = path.join(__dirname, '../../../../build/Release/secure_ipc.node');
  secure_ipc = require(devPath);
} catch {
  try {
    const prodPath = path.join(process.resourcesPath || '', 'app.asar.unpacked/build/Release/secure_ipc.node');
...
const isSocketActivated = listenFdStr !== undefined && /^\d+$/.test(listenFdStr);
...
const server = net.createServer((socket) => {
  ...
  socket.on('data', async (data) => {
    ...
    if (buffer.length > MAX_BUFFER_SIZE) {
      console.error('SECURITY: IPC buffer exceeded 1MB limit. Disconnecting client.');
      socket.destroy();
      return;
    }
    ...
    while ((newlineIdx = buffer.indexOf('\n')) !== -1) {
      ...
      try {
        ...
        if (req.type === 'process' && req.file && typeof req.file.path === 'string') {
          ...
          const callerCanRead = secure_ipc.checkAccessAsUser(targetPath, peerUID);
          if (!callerCanRead) {
...
if (isSocketActivated) {
  ...
  server.listen({ fd }, () => {
    console.log(`Daemon listening on inherited fd ${fd} (socket-activated by launchd)`);
  });
} else {
